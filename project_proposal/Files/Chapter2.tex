...
\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{Figures/Ripe.jpeg}
\caption{Ripe coffee berries showing color variation from green to deep red}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.70\textwidth]{Previous_Model_F1__score.PNG}
...
\textbf{1. Labor Intensity and Fatigue:} Manual sorting requires a large workforce to maintain reasonable throughput. Studies show that prolonged visual inspection leads to fatigue, which directly reduces sorting accuracy and consistency \cite{brosnan2004}. Fatigued workers are more likely to misclassify berries, allowing defective berries to pass through or rejecting acceptable ones.

\textbf{2. Human Error and Subjectivity:} Manual sorting depends heavily on individual judgment, which varies between workers and across time \cite{blasco2003}. Significant variability in sorting outcomes has been observed across different shifts, even when standardized grading guidelines were provided. This subjectivity results in inconsistent quality between batches.

\textbf{3. Limited Throughput:} Human sorting speed is inherently constrained. Manual sorting becomes a bottleneck during peak harvest seasons, limiting scalability and increasing processing delays \cite{kumar2010}. This inefficiency directly affects farmers' ability to meet market demand.
...
Automated coffee berry sorting systems offer several key advantages over manual methods \cite{cubero2011}:
...
\begin{figure}[h]
\centering
\includegraphics[width=0.82\textwidth]{Figures/sorting_comparison.pdf}
\caption{Comparison of manual and electronic coffee sorting methods (Source: Agriculture Institute, 2024)}
\label{fig:separation_methods}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{Fully_Fabricated_Assembbly_From_Previous_Group.PNG}
\caption{Fully fabricated mechanical assembly from previous implementation \cite{mugo2017}, showing integrated hopper, vibrating feeder, chute, and frame structure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.82\textwidth]{Figures/classification_flowchart.pdf}
\caption{Overview of classification methods used in coffee berry sorting systems}
\label{fig:classification_methods}
\end{figure}

\subsubsection{Color-Based Classification}

Color-based classification is the most widely used method in coffee berry sorting due to its direct correlation with maturity \cite{gonzalez2018}. Ripe coffee berries exhibit deep red or purple hues, while unripe berries appear green or yellow. Systems employ RGB cameras to capture color information, which is then analyzed using color histograms, color space transformations (e.g., HSV, Lab), or simple thresholding.
...
Shape analysis assesses geometric attributes such as size, contour smoothness, aspect ratio, and circularity \cite{szeliski2011}. Machine vision systems extract berry contours from binary images and calculate shape descriptors to identify malformed, damaged, or insect-infested berries.
...
Deep learning approaches, particularly Convolutional Neural Networks (CNNs), have demonstrated superior classification accuracy by automatically learning discriminative features from labeled training data \cite{zhang2018}. Architectures such as MobileNet \cite{howard2017mobilenets}, ResNet, and YOLO (You Only Look Once) \cite{redmon2018yolo} enable real-time object detection and classification.

...
The classification system will be designed as a binary classifier to distinguish between **ripe** and **unripe** coffee berries. This simplified two-class approach offers several advantages:

\begin{itemize}
    \item Reduced computational complexity suitable for real-time processing on embedded platforms
    \item Faster training convergence with smaller labeled datasets
    \item Higher classification accuracy due to clearer decision boundaries
    \item Practical alignment with primary sorting objectives in coffee processing
\end{itemize}

The model will be trained using a curated dataset of coffee berry images labeled as either ripe (deep red/purple berries suitable for processing) or unripe (green, yellow, or partially colored berries requiring rejection). A lightweight convolutional neural network (CNN) architecture such as MobileNetV2 will be employed and optimized using TensorFlow Lite for deployment on the Raspberry Pi.

Training will involve data augmentation techniques including rotation, brightness adjustment, and horizontal flipping to improve model generalization. The trained model will be quantized to reduce inference time while maintaining classification accuracy above the 90\% threshold.
